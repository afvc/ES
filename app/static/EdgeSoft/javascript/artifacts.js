...
function createBranchesButton(){
	...
	document.getElementById("Lista").addEventListener("click", viewBtnHandlerLista);
	document.getElementById("Icones").addEventListener("click", viewBtnHandlerIcones);
	xmlStart2();
	//console.log("project_id = " + project_id + " - " + private_token);
...
function viewBtnHandlerIcones(){
	viewControl=1;
	update();
}

function viewBtnHandlerLista(){
	viewControl=0;
	update();
}
...
function getRepositoryTree() {
	...
	if (path == "undefined") {
		...
		//console.log("project_id = " + project_id + " path: " + path + " branch: " + branch);
...
function createFiles() {
	...
	for (var i = 0; i < JsonObject.length; i++) {
		...
		if (JsonObject[i].type == "tree"){
			img.src = "../static/EdgeSoft/imgs/pasta.png";

		} 
		else img.src = "../static/EdgeSoft/imgs/iconficheiro.png";
		$(img).css({
			"width": "30px"
			, "height": "30px"
			, "position": "absolute"
